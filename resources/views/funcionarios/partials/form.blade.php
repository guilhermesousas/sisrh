<div class="col-md-6">
  ...
  <div class="col-md-4">
    ...
    <select id="departamento_id" name="departamento_id" class="form-select" value="{{$funcionario->departamento ?? ""}}" required>
        ...
        <option value="{{$departamento->id}}"
            @if(isset($funcionario->departamento_id))@selectd($funcionario->departamento->id == $departamento->id)@endif>{{$departamento->nome}}
        </option
        >@endforeach
    </select>
  </div>
  <div class="col-md-4">
    ...
    <select type="text" class="form-control" id="cargo_id" name="cargo_id" required>
        ...
        <option value="{{ $cargo->id }}"
            @if(isset($funcionario->cargo_id))@selectd($funcionario->cargo->id == $cargo->id)@endif>{{$cargo->nome}}
        </option>
        @endforeach
    </select>
  </div>
  ...
  <div class="col-md-4">
    ...
    <input type="date" class="form-control" id="data_desligamento" name="data_desligamento" value= "{{$funcionario->data_desligamento ?? ""}}"required>
  </div>
  <div class="col-md-2">
